...
export default function AdminInvoicesPage() {
  ...
  const filtered = invoices.filter(
    (i) => i.id.includes(search) || i.customer.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleDownload = (inv) => {
    ...
    // In production: call invoicesAPI.download(inv.id) and trigger blob download
  };
        ...
        <Box sx={{ overflowX: 'auto' }}>
          <Table>
            ...
            <TableBody>
              {paginated.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} align="center" sx={{ py: 6, color: 'text.secondary' }}>No invoices found.</TableCell>
                </TableRow>
              ) : (
                paginated.map((inv) => (
                  <TableRow key={inv.id}>
                    <TableCell sx={{ fontWeight: 700, color: '#1B4332', fontSize: 13 }}>{inv.id}</TableCell>
                    <TableCell sx={{ color: '#0369A1', fontWeight: 600, fontSize: 13 }}>{inv.order}</TableCell>
                    <TableCell sx={{ fontSize: 13 }}>{inv.customer}</TableCell>
                    <TableCell sx={{ fontWeight: 700 }}>₹{inv.amount}</TableCell>
                    <TableCell sx={{ fontSize: 13, color: 'text.secondary' }}>{inv.date}</TableCell>
                    <TableCell>
                      <Chip label={inv.status} size="small" sx={{ ...statusColors[inv.status], fontWeight: 700, borderRadius: 1.5, fontSize: 11 }} />
                    </TableCell>
                    <TableCell align="center">
                      <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'center' }}>
                        <Tooltip title="Download">
                          <IconButton size="small" onClick={() => handleDownload(inv)} sx={{ color: '#1B4332' }}>
