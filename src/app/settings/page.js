    ...
    <AdminShell>
      <Paper sx={{ borderRadius: 3, boxShadow: '0 4px 20px rgba(0,0,0,0.07)', overflow: 'hidden' }}>
        <Tabs
          ...
          sx={{ borderBottom: '1px solid #E7E5E4', px: 3 }}
        >
          <Tab label="Profile" />
          <Tab label="Password" />
        ...
        <Box sx={{ p: 3 }}>
          ...
          <TabPanel value={tab} index={0}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, mb: 4 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                  {state.admin?.role?.replace('_', ' ') || 'Admin'} · {profile.email}
                </Typography>
              </Box>
            </Box>
            <Grid container spacing={3}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  fullWidth label="Phone" value={profile.phone}
          ...
          <TabPanel value={tab} index={1}>
            <Grid container spacing={3} sx={{ maxWidth: 480 }}>
              <Grid size={12}>
                <TextField
                  fullWidth label="Current Password" type="password" value={passwords.current}
                  onChange={(e) => setPasswords({ ...passwords, current: e.target.value })}
                />
              </Grid>
              <Grid size={12}>
                <TextField
                  fullWidth label="New Password" type="password" value={passwords.newPwd}
                  onChange={(e) => setPasswords({ ...passwords, newPwd: e.target.value })}
                />
              </Grid>
              <Grid size={12}>
                <TextField
                  fullWidth label="Confirm New Password" type="password" value={passwords.confirm}
          ...
          <TabPanel value={tab} index={2}>
            <Grid container spacing={3}>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  fullWidth label="Site Name" value={siteSettings.siteName}
                  onChange={(e) => setSiteSettings({ ...siteSettings, siteName: e.target.value })}
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  fullWidth label="Support Email" value={siteSettings.supportEmail}
                  onChange={(e) => setSiteSettings({ ...siteSettings, supportEmail: e.target.value })}
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  fullWidth label="Currency" value={siteSettings.currency}
                  onChange={(e) => setSiteSettings({ ...siteSettings, currency: e.target.value })}
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  fullWidth label="Low Stock Alert Threshold" type="number"
                  value={siteSettings.lowStockThreshold}
                  onChange={(e) => setSiteSettings({ ...siteSettings, lowStockThreshold: e.target.value })}
                />
              </Grid>
