...
import { Lock, Visibility, VisibilityOff, ArrowBack, AccessTime } from '@mui/icons-material';
...
function ResetPasswordForm() {
  ...
  const validate = () => {
    ...
    else if (!/^\d{6}$/.test(form.otp))                      errors.otp             = 'OTP must be exactly 6 digits';
  ...
  const handleSubmit = async (e) => {
    ...
    setGeneralError('');
    setFieldErrors({});
    if (timerExpired) { setGeneralError('OTP has expired. Please request a new one from Forgot Password.'); return; }
    ...
    try {
      await resetPasswordRequest({ email: form.email, otp: form.otp, newPassword: form.newPassword, confirmPassword: form.confirmPassword });
      ...
      <Container maxWidth="xs" sx={{ position: 'relative', zIndex: 1 }}>
        <Paper sx={{ p: { xs: 3, sm: 5 }, borderRadius: 4, boxShadow: '0 20px 60px rgba(27,67,50,0.15), 0 4px 16px rgba(0,0,0,0.08)', border: '1px solid rgba(27,67,50,0.08)' }}>
          <Box sx={{ textAlign: 'center', mb: 3 }}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 800, color: '#1B4332' }}>Reset Password</Typography>
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>Enter the OTP sent to your email</Typography>
          </Box>
          ...
          {timerExpired && (
            <Alert severity="error" sx={{ mb: 2, borderRadius: 2 }}>
              OTP has expired.{' '}
              <Link href="/forgot-password" style={{ color: 'inherit', fontWeight: 700 }}>Request a new one</Link>
            </Alert>
          )}
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate>
            ...
            <TextInput
              ...
              slotProps={{
                htmlInput: { inputMode: 'numeric', maxLength: 6, style: { letterSpacing: '0.4em', fontWeight: 700, fontSize: 20 } },
              }}
              sx={{ mb: 2.5 }}
            />
            ...
            <TextInput
              ...
              error={fieldErrors.newPassword || (!fieldErrors.newPassword ? 'Minimum 6 characters' : '')}
              slotProps={{
                input: {
                  startAdornment: <InputAdornment position="start"><Lock color="action" /></InputAdornment>,
                  endAdornment:   newPasswordEnd,
                },
              }}
              sx={{ mb: 2.5 }}
            />
            ...
            <TextInput
              ...
              slotProps={{
                input: {
                  startAdornment: <InputAdornment position="start"><Lock color="action" /></InputAdornment>,
                  endAdornment:   confirmPasswordEnd,
                },
              }}
              sx={{ mb: 3 }}
            />

            <Button type="submit" fullWidth variant="contained" size="large" disabled={submitting || timerExpired}
