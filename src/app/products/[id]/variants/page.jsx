    ...
    <AdminShell>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Stack direction="row" spacing={2} sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
          {/* Search */}
          <TextInput size="small" placeholder="Search variants…" value={search}
      ...
      <Table
        columns={[
          ...
          { key: 'weight', label: 'Weight',
            render: (row) => <Typography sx={{ fontSize: 13 }}>{row.weight ? `${row.weight} ${row.weight_unit || ''}`.trim() : '—'}</Typography> },
        ...
        emptyMessage={search ? `No variants found for "${search}"` : 'No variants yet'}
        count={count}
        page={pageValue}
        rowsPerPage={limit}
